...
void GPIO_Init(void)
{
    /* Enable P9.7 interrupt by falling edge trigger */
    P9->IMD |= (GPIO_IMD_EDGE << 7);
    P9->IEN |= (BIT7 << GPIO_IEN_IF_EN_Pos);
...
void GPG1_IRQHandler(void)
{
    ...
    if(GPIO_GET_INT_FLAG(P9, BIT7))
    {
        GPIO_CLR_INT_FLAG(P9, BIT7);
